...
process.env.IAGOTV_AUTH_SECRET = "test-only-secret-that-is-longer-than-32-bytes";
...
test("access and refresh tokens cannot be exchanged across token types", () => {
  ...
  const access = security.signIagotvAccessToken(account);
  const refresh = security.signIagotvRefreshToken(account);

  assert.equal(security.verifyIagotvAccessToken(access).supabaseUserId, "account-123");
  assert.equal(security.verifyIagotvRefreshToken(refresh).email, "user@example.org");
  assert.throws(() => security.verifyIagotvAccessToken(refresh), /token type/i);
  assert.throws(() => security.verifyIagotvRefreshToken(access), /token type/i);
});
