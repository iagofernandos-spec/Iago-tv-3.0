...
const baseAddress = String(process.env.IAGOTV_DELETION_TEST_EMAIL || "").trim().toLowerCase();
if (!baseAddress.includes("@")) {
  console.error("Set IAGOTV_DELETION_TEST_EMAIL to a real inbox that accepts plus aliases.");
  process.exit(1);
}

const [localPart, domain] = baseAddress.split("@");
const email = `${localPart}+iagotv-delete-${Math.floor(Date.now() / 1000)}@${domain}`;
...
const functionBase = "https://auth.iagotv.app/.netlify/functions";
...
const stores = {
  auth: getStore({ name: "iagotv-auth", siteID, token: netlifyToken }),
