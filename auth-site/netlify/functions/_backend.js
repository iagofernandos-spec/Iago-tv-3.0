...
const JSON_HEADERS = {
  ...
  "access-control-allow-headers": "authorization,apikey,content-type,x-iagotv-user-id,x-iagotv-email,x-client-info,x-user-token",
...
const AUTH_ISSUER = "iagotv-netlify";
...
function authSecret() {
  const secret = process.env.IAGOTV_AUTH_SECRET || "";
  if (!secret || secret.length < 32) {
    const error = new Error("IAGOTV_AUTH_SECRET is not configured");
...
function signIagotvToken(account, tokenType, ttlSeconds) {
...
function signIagotvAccessToken(account) {
  return signIagotvToken(account, "access", ACCESS_TOKEN_TTL_SECONDS);
}

function signIagotvRefreshToken(account) {
  return signIagotvToken(account, "refresh", Math.floor(REFRESH_TOKEN_TTL_MS / 1000));
}

function verifyIagotvToken(token, expectedType) {
  const parts = String(token || "").split(".");
  if (parts.length !== 3) {
    throw new Error("Invalid IagoTV token");
  }
  ...
  if (
    ...
    throw new Error("Invalid IagoTV token signature");
  }
  ...
  try {
    ...
    throw new Error("Invalid IagoTV token payload");
  }
  if (payload.iss !== AUTH_ISSUER || !payload.sub || !payload.email) {
    throw new Error("Invalid IagoTV token claims");
  }
  const tokenType = payload.token_type || "access";
  if (tokenType !== expectedType) throw new Error("Invalid IagoTV token type");
  if (Number(payload.exp || 0) <= Math.floor(Date.now() / 1000)) {
    throw new Error("IagoTV token expired");
  }
  return {
...
function verifyIagotvAccessToken(accessToken) {
  return verifyIagotvToken(accessToken, "access");
}

function verifyIagotvRefreshToken(refreshToken) {
  return verifyIagotvToken(refreshToken, "refresh");
}
...
async function issueIagotvSession(event, account) {
  const normalizedAccount = {
    ...account,
    email: normalizeEmail(account.email)
  };
  const accessToken = signIagotvAccessToken(normalizedAccount);
  const refreshToken = signIagotvRefreshToken(normalizedAccount);
  return {
...
async function refreshIagotvSession(event, refreshToken) {
  if (String(refreshToken || "").split(".").length === 3) {
    let identity;
    try {
      identity = verifyIagotvRefreshToken(refreshToken);
    } catch (error) {
    ...
    return issueIagotvSession(event, account);
  }
  ...
  const replacement = await issueIagotvSession(event, account);
  await Promise.all([
    store.delete(refreshKey).catch(() => {}),
    store.delete(refreshAccountReferenceKey(session.accountId, refreshToken)).catch(() => {})
  ]);
...
async function sendTransactionalEmail(email, subject, text, html) {
  ...
  const from = process.env.AUTH_EMAIL_FROM || "IagoTV <noreply@auth.iagotv.app>";
  if (provider === "resend") {
  ...
  if (provider === "sendgrid") {
    const response = await fetch("https://api.sendgrid.com/v3/mail/send", {
      ...
      body: JSON.stringify({
        ...
        from: { email: from.replace(/^.*<(.+)>$/, "$1"), name: "IagoTV" },
        subject,
        content: [
...
async function sendPasswordSetupEmail(email, setupUrl) {
  const subject = "Create your IagoTV Cloud password";
  const text = [
    "IagoTV Cloud moved to a new secure server.",
    "To keep your account protected, create a new IagoTV Cloud password:",
    setupUrl,
    "This link expires in 1 hour."
  ].join("\n\n");
  const html = `
    <p>IagoTV Cloud moved to a new secure server.</p>
    <p>To keep your account protected, create a new IagoTV Cloud password:</p>
...
async function sendAccountDeletionConfirmation(email) {
  const subject = "Your IagoTV account has been deleted";
  const text = [
    "Your IagoTV account and cloud-synced data have been permanently deleted.",
    "No further action is required.",
    "If you did not request this deletion, contact iagotv.app@gmail.com."
  ].join("\n\n");
  const html = `
    <p>Your IagoTV account and cloud-synced data have been permanently deleted.</p>
    <p>No further action is required.</p>
    <p>If you did not request this deletion, contact <a href="mailto:iagotv.app@gmail.com">iagotv.app@gmail.com</a>.</p>
  `;
  return sendTransactionalEmail(email, subject, text, html);
}
...
async function startPasswordSetup(event, email) {
  ...
  const baseUrl = (process.env.SITE_URL || process.env.TV_AUTH_VERIFY_BASE_URL || "https://auth.iagotv.app").replace(/\/+$/, "");
...
async function authenticateNetlifyPassword(event, email, password) {
  ...
  if (requiresExplicitPasswordSetup(account)) {
    await throwPasswordSetupRequired(
      event,
      email,
      "IagoTV Cloud moved to a new secure server. To keep your data protected, create a new IagoTV Cloud password from the email we sent you."
    );
  }
...
async function createNetlifyAccount(event, email, password) {
  ...
  if (requiresExplicitPasswordSetup(existing)) {
    await throwPasswordSetupRequired(
      event,
      email,
      "IagoTV Cloud moved to a new secure server. Create a new IagoTV Cloud password to keep your existing data."
    );
  }
...
async function handleAuthRefresh(event) {
  ...
  try {
    ...
    const token = await refreshIagotvSession(event, refreshToken);
    return json(200, token);
  } catch (error) {
...
async function handleTvAuthStart(event) {
  ...
  try {
    ...
    const verifyBase = (process.env.TV_AUTH_VERIFY_BASE_URL || process.env.SITE_URL || "https://auth.iagotv.app").replace(/\/+$/, "");
    return json(200, {
  ...
  try {
    ...
    const response = await fetch(tmdbUrl, {
      headers: {
        ...
        "user-agent": "IagoTV-Netlify-TMDB-Proxy/1.0"
      }
    });
...
async function resolveIdentity(event) {
  ...
  try {
    const identity = verifyIagotvAccessToken(token);
...
async function getOrCreateAccount(client, identity) {
  ...
  const inserted = await client.query(
    `INSERT INTO public.iagotv_accounts (email, email_normalized, supabase_user_id, last_seen_at)
...
async function deleteDatabaseAccount(email, accountId) {
  ...
  try {
    ...
    const accounts = await client.query(
      `DELETE FROM public.iagotv_accounts
        WHERE supabase_user_id::text = $1 OR email_normalized = $2`,
      [String(accountId), normalizeEmail(email)]
    );
...
module.exports = {
  ...
  handleTvAuthStatus,
  _test: {
    signIagotvAccessToken,
    signIagotvRefreshToken,
    verifyIagotvAccessToken,
    verifyIagotvRefreshToken,
